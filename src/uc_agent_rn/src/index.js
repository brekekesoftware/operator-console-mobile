...
uiData.prototype.changeLamp = function () {
  ...
  var blinkingCount = Object.keys(this.blinkingTabs).reduce(
    (a, k) => a + _this4.blinkingTabs[k],
    0,
  )
...
uiData.prototype.updateTab = function (option) {
  ...
  if (option) {
    if (option.open) {
      if (
        ...
        ;(function () {
          ...
          var sourcePanelIndex = _this5.mainPanelList.findIndex(
    ...
    if (option.close) {
      var index = this.mainPanelList.findIndex(
        p =>
      ...
      if (ev.phone_prop) {
        ;(function () {
          ...
          try {
            ...
            Object.keys(phone_prop).forEach(
              k => (_this8.phone[k] = phone_prop[k]),
            )
          } catch (ex) {
      ...
      Object.values(this.ucUiStore.getChatTable()).reduce(
        (accumulator, currentValue) => {
          ...
          var conference =
            ...
            _this9.ucUiStore
              .getChatClient()
              .getConference(chatHeaderInfo.conf_id)
          if (
            conference &&
            conference.conf_status === _constants2.default.CONF_STATUS_JOINED &&
            conference.user.filter(
              u => u.conf_status === _constants2.default.CONF_STATUS_JOINED,
            ).length >= 2
          ) {
...
uiData.prototype.setPhoneDefaultOptions = function () {
  ...
  var webRTCType = webRTCTypes.find(type => type.name === webRTCTypeName)
  if (webRTCType) {
    try {
...
uiData.prototype.getlampTypeOptions = function () {
  ...
  var lampType = lampTypes.find(type => type.name === lampTypeName)
  if (lampType) {
    try {
      ...
      ev => {
        if (!ev.warnings) {
          if (!invitesSoon) {
            ...
            _this11.updateTab({
              open: { panelType: 'CONFERENCE', panelCode },
              select: {
...
uiData.prototype.sidebarBuddylistItem_onClick = function (buddy, ev) {
  ...
  this.updateTab({
    open: { panelType, panelCode },
    select: { panelType, panelCode },
  })
}
...
uiData.prototype.sidebarEditStatusDisplayButton_onClick = function (ev) {
  ...
  this.showModal({
    ...
    onOk: function onOk(ev) {
      ...
      // change
      _this12.ucUiAction.changeStatus({ display })
      // add to history
      if (display) {
...
uiData.prototype.sidebarControlProfileStatusItem_onClick = function (
  ...
  this.ucUiAction.changeStatus({ status, display: '' })
}
  ...
  this.showModal({
    ...
    onOk: function onOk(ev) {
      ...
      if (text) {
        ...
        _this15.updateTab({
          open: { panelType: 'EXTERNALCALL', panelCode },
          select: { panelType: 'EXTERNALCALL', panelCode },
        })
      } else if (ev) {
        ev.closes = false
      }
  ...
  allUsers.user.forEach(u => {
    ...
    if (
      contentParams.buddylist.user.find(
        b => b.tenant === profile.tenant && b.user_id === u.user_id,
      )
    ) {
      // already exists
      return
          ...
          if (!ev.content.state.saveOrder) {
            ;(function () {
              ...
              newBuddylist.user.forEach(buddy => {
                if (buddy.user_id) {
                  var editedBuddy = editedBuddies.find(
...
uiData.prototype.tabLinkHideButton_onClick = function (
  ...
  this.closeTab({ panelType, panelCode, ev })
}
      ...
      if (conference.conf_status === _constants2.default.CONF_STATUS_JOINED) {
        if (conference.conf_type === 'webchat') {
          ...
          showModal({
            ...
            cancelable: true,
            onOk: funcConfirmClosable.bind(_this20, () => {
...
uiData.prototype.tabLinkMoveHContextMenuItem_onClick = function (
  ...
  var panel = this.mainPanelList.find(
  ...
  if (dragSourceInfoType === 'mainTabLinkSpan') {
    try {
      ;(function () {
        ...
        sourceIndex = _this21.mainPanelList.findIndex(
  ...
  if (dropTargetInfoType === 'mainTabLinkSpan') {
    try {
      ;(function () {
        ...
        targetIndex = _this21.mainPanelList.findIndex(
...
uiData.prototype.panelHeaderUndockButton_onClick = function (
  ...
  var subWindow = this.subWindowList.find(
    p => p.panelType === panelType && p.panelCode === panelCode,
  )
  if (subWindow) {
...
uiData.prototype.subwindow_unload = function (panelType, panelCode, ev) {
  var index = this.subWindowList.findIndex(
    p => p.panelType === panelType && p.panelCode === panelCode,
  )
  if (index !== -1) {
...
uiData.prototype.webchatPickupButton_onClick = function (data, ev) {
  ...
  var webchatQueue = this.ucUiStore.getWebchatQueueList().find(webchatQueue => {
  ...
  if (webchatQueue) {
    ...
    this.updateTab({
      select: {
        panelType: 'CONFERENCE',
        panelCode: (0, _strings.string)(
          this.ucUiStore.getChatCodeByConfId({ conf_id }).chatCode,
        ),
      },
    })
    this.render()
...
uiData.prototype.webchatRoomJoinButton_onClick = function (conf_id, ev) {
  ...
  this.closeAllshowingDialogs()
  this.updateTab({
    select: { panelType: 'CONFERENCE', panelCode },
  })
  this.render()
}
...
uiData.prototype.webchatRoomHideButton_onClick = function (conf_id, ev) {
  this.ucUiAction.clearWebchatQueue({ conf_id })
}
    ...
    if (
      ...
      ;(function () {
        ...
        _this37.outgoingContinuationInfos =
          _this37.outgoingContinuationInfos.filter(info => {
            if (
      ...
      if (
        ...
        ;(function () {
          ...
          var _loop = function _loop(i) {
            panelToNotify = _this39.mainPanelList.find(
  ...
  if (rtcSession) {
    var _ret32 = (function () {
      ...
      if (rtcSession.direction === 'outgoing') {
        ...
        var panelCode = void 0
        var webchatQueue = void 0
        if (
          (buddy =
            address &&
            (_this40.ucUiStore.getChatClient().getBuddylist().user || []).find(
              b => address === (0, _strings.string)(b && b.user_id),
            ))
        ) {
          panelType = 'CHAT'
      ...
      if (rtcSession.direction === 'outgoing') {
        ...
        if (address) {
          Object.keys(_this41.panelSessionTable)
            ...
            .forEach(panelKey => delete _this41.panelSessionTable[panelKey])
        }
      } else {
        // incoming
        var panelType = void 0
...
uiData.prototype.rtcErrorOccurred = function (ev) {
  ...
  if (ev && ev.target && !ev.sessionId) {
    ...
    Object.keys(this.panelSessionTable)
      ...
      .forEach(panelKey => delete _this45.panelSessionTable[panelKey])
    warningMessageKey = 'MSG_CALL_RTC_ERROR'
  } else if (ev && ev.sessionId) {
    panelKey = Object.keys(this.panelSessionTable)
      .filter(
        panelKey =>
          _this45.panelSessionTable[panelKey].sessionId === ev.sessionId,
      )
      .pop()
...
uiData.prototype.icegatheringstatechange = function (ev) {
  ...
  var panelKey =
    ...
    Object.keys(this.panelSessionTable)
      .filter(
        panelKey => _this46.panelSessionTable[panelKey].sessionId === sessionId,
      )
      .pop()
...
uiData.prototype.iceconnectionstatechange = function (ev) {
  ...
  var panelKey =
    ...
    Object.keys(this.panelSessionTable)
      .filter(
        panelKey => _this47.panelSessionTable[panelKey].sessionId === sessionId,
      )
      .pop()
...
AgentComponent.prototype.newChat = function (conf_id) {
  ...
  var isTalking =
    conference.conf_type === 'webchat'
      ? this.ucUiStore && this.ucUiStore.getWebchatQueue({ conf_id }).isTalking
      : conference.conf_status === _constants2.default.CONF_STATUS_JOINED
...
AgentComponent.prototype.hideSearchDialog = function (option) {
  ...
  if (option.clear) {
    ...
    var searchConditionsNew = searchConditions.map(searchCondition => {
        ...
        var _ret36 = (function () {
          ...
          if (
            (
              ...
              aws => aws.id === searchResult.webchatServiceId && aws.senders,
            )
          ) {
            var replyType = null
...
AgentComponent.prototype.chatClient_conferenceMemberChanged = function (ev) {
  ...
  if (conf_id && this.chatTable[conf_id]) {
    ...
    var isTalking =
      conference.conf_type === 'webchat'
        ? (this.ucUiStore &&
            this.ucUiStore.getWebchatQueue({ conf_id }).isTalking) ||
          (conference.creator &&
...
AgentComponent.prototype.chatClient_confTagUpdated = function (ev) {
  ...
  if (conf_id && this.chatTable[conf_id]) {
    ...
    this.chatTable[conf_id].extWebchatInfo =
      ...
      {}
    this.cacheConference(conf_id, { conference })
    ...
    ;(function () {
      ...
      if (continuationInfo.conf_id) {
        ...
        _this54.replyingContinuationInfos =
          _this54.replyingContinuationInfos.filter(info => {
            if (
              info.conf_id === continuationInfo.conf_id &&
              info.yyyymm === continuationInfo.yyyymm
            ) {
...
AgentComponent.prototype.subWindowChatOptionButtonsReplyWebchatButton_onClick =
  function (panelType, panelCode, ev) {
    ...
    var e = { webchatId, cancel: false, option }

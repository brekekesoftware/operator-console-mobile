...
;(function (root, factory) {
  ...
  ChatClient.prototype = {
    ...
    signIn: function (
      ...
      this._enableDisconnectedStatus = Boolean(option.enableDisconnectedStatus)
      this._sessAttr =
        '' +
          ...
          if (index !== -1) {
            ...
            if (object && typeof object === 'object') {
              if (
                ...
                content = content.substring(0, index) + JSON.stringify(object)
              }
            }
          }
      ...
      if (this._signInStatus === 2) {
        if (this._mfa_verification_required) {
          ...
          if (now < this._mfa_verification_required) {
            this._logger.log(
              'info',
              '_signInTimeout() expiration=' + this._mfa_verification_required,
            )
            setTimeout(
              this._byThis(this._signInTimeout),
            ...
            this._signInNG(Errors.SIGN_IN_TIMEOUT, 'Verification timeout')
          }
        } else {
    ...
    _signInStartRpc: function (password) {
      ...
      if (this._device_token) {
        prms += '&device_token=' + encodeURIComponent(this._device_token)
      }
    ...
    _checkTimezoneOffset: function () {
      ...
      if (nowTime >= this._timezoneOffsetNextCheckTime) {
        ...
        var ucUrlFromClient =
          ...
          '/' +
          this._path
        if (settings.optional_settings.uc_url_from_client !== ucUrlFromClient) {
          settings.optional_settings.uc_url_from_client = ucUrlFromClient
          modified = true
        }
    ...
    _recvNotifyStatus2: function (args, user, isNonbuddy) {
      ...
      this._buddyStatus[key] = {
        ...
        disconnected_display: string(args.client_param.disconnected_display),
      }
      // memory download key
